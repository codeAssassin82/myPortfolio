 ...
 *
 */
if( ! function_exists('mi_additional_menu') ) {
  function mi_additional_menu($class = '') {

    if ( function_exists('wp_nav_menu') && has_nav_menu( 'additional-menu' ) ) {
 ...
 *
 */
if( ! function_exists( 'mi_set_wpautop' ) ) {
  function mi_set_wpautop( $content, $force = true ) {
    if ( $force ) {
 ...
 *
 */
if ( ! function_exists( 'mi_element_values' ) ) {
  function mi_element_values(  $type = '', $query_args = array() ) {
 ...
 *
 */
if( ! function_exists( 'mi_get_first_url_from_string' ) ) {
  function mi_get_first_url_from_string( $string ) {
 ...
 *
 */
if( ! function_exists('mi_get_shortcode_regex') ) {
  function mi_get_shortcode_regex( $tagregexp = '' ) {
 ...
 *
 */
if( ! function_exists( 'mi_tagregexp' ) ) {
  function mi_tagregexp() {
    return apply_filters( 'mi_custom_tagregexp', 'video|audio|playlist|video-playlist|embed|cs_media' );
  }
}
...
if( ! function_exists('mi_is_explodable' ) ) {
  function mi_is_explodable( $page_name ) {
 ...
 *
 */
if( ! function_exists( 'mi_post_media' ) ) {
  function mi_post_media( $content ) {

    $is_video = ( get_post_format() == 'video' ) ? true : false;
    $media    = mi_get_first_url_from_string( $content );
    ...
    if( ! empty( $media ) ) {
      ...
      $pattern = mi_get_shortcode_regex( mi_tagregexp() );
    ...
    if( ! empty( $media ) ) {
      ...
      $output .= ( $is_video ) ? '<div class="mi-fluid"><div class="mi-fluid-inner">' : '';
      $output .= $content;
